...
def run_schedule():
    ...
    while should_run:
        ...
        if not config['enable_scraping']:
            should_run = False
            print("Stopping scheduler as enable_scraping is set to False.")
        else:
...
def setup_scheduler():
    config = config_manager.load()
    if config['enable_scraping']:
        schedule.every(1).second.do(scrapping)
        run_schedule()
    else:
        print("Scheduler not set up as enable_scraping is set to False.")
def stop_thread():
    global should_run
    should_run = False
...
if __name__ == "__main__":
    scrapper=scrapping_init()

    t = Thread(target=setup_scheduler)
    t.start()
